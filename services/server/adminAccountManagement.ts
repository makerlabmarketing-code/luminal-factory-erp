...
function activePermissionCount(employee: EmployeeAccountRow, rows: PermissionRow[]): number {
...
function detectPreset(
  ...
  const activePermissionStates = new Map<PermissionCode, PermissionEditorState>();
  ALL_PERMISSION_CODES.forEach((permissionCode) => {
    activePermissionStates.set(permissionCode, permissionStateFor(employee, permissionRows, permissionCode));
  });

  const matchedPreset = ACCOUNT_PRESETS.find((preset) => {
...
function resolveAccountStatus(
  ...
  if (!hasWorkspace(employee, workspaceRows, 'STAFF_WORKSPACE') && !hasWorkspace(employee, workspaceRows, 'ADMIN_WORKSPACE')) {
...
async function loadAccountRows() {
  const supabaseAdmin = createSupabaseAdminClient();
  const [{ data: employees, error: employeeError }, { data: workspaceRows, error: workspaceError }, { data: permissionRows, error: permissionError }] =
    await Promise.all([
      supabaseAdmin
        .from('employees')
  ...
  if (employeeError || workspaceError || permissionError) {
    throw new AuthFlowError({
      ...
      safeDetails: {
        supabase_error_code:
          employeeError?.code || workspaceError?.code || permissionError?.code || 'unknown',
      },
    });
  }
...
async function loadTargetEmployee(employeeIdValue: string): Promise<EmployeeAccountRow> {
  const supabaseAdmin = createSupabaseAdminClient();
...
function makeNextWorkspaceRows(
  ...
  const nextRows = rows.filter((row) => String(row.employee_id) !== targetEmployeeId);
...
async function grantWorkspace(
  ...
  const { error } = await supabaseAdmin.from('employee_workspace_access').insert({
    employee_id: targetEmployeeId,
    workspace,
    status: 'ACTIVE',
...
async function revokeWorkspace(targetEmployeeId: string, workspace: WorkspaceCode) {
  const supabaseAdmin = createSupabaseAdminClient();
...
async function setPermissionState(
  ...
  const effectsToRevoke: PermissionEffect[] =
    state === 'ALLOW' ? ['DENY'] : state === 'DENY' ? ['ALLOW'] : ['ALLOW', 'DENY'];
...
function parsePermissionState(value: unknown): PermissionEditorState | null {
  return value === 'ALLOW' || value === 'DENY' || value === 'NONE' ? value : null;
}
...
export async function getAdminAccountManagementData(): Promise<AdminAccountManagementData> {
  ...
  return {
    accounts: employees.map((employee) =>
      toAccountListItem(
        employee,
        employee.auth_user_id ? authUsersById.get(employee.auth_user_id) || null : null,
        workspaceRows,
        permissionRows,
...
export async function getAdminAccountDetailData(employeeIdValue: string): Promise<AdminAccountDetailDto> {
  const authContext = await requireAccountManager();
...
export async function updateAccountWorkspaces(
  ...
  const authContext = await requireAccountManager();
  await loadTargetEmployee(targetEmployeeId);
  ...
  const nextWorkspaceRows = makeNextWorkspaceRows(workspaceRows, targetEmployeeId, desiredWorkspaces);

  await assertNotRemovingLastAdministrator(targetEmployeeId, nextWorkspaceRows, permissionRows);

  if (staffWorkspace) await grantWorkspace(targetEmployeeId, 'STAFF_WORKSPACE', employeeId(authContext));
  else await revokeWorkspace(targetEmployeeId, 'STAFF_WORKSPACE');

  if (adminWorkspace) await grantWorkspace(targetEmployeeId, 'ADMIN_WORKSPACE', employeeId(authContext));
...
export async function updateAccountPermissions(
  ...
  for (const permissionCode of ALL_PERMISSION_CODES) {
    const state = parsePermissionState((permissionPayload as Record<string, unknown>)[permissionCode]);
    if (!state) {
      throw new AuthFlowError({
        status: 400,
        code: 'employee_not_linked',
  ...
  const { workspaceRows, permissionRows } = await loadAccountRows();
  const nextPermissionRows = makeNextPermissionRows(permissionRows, targetEmployeeId, desiredPermissions);
  await assertNotRemovingLastAdministrator(targetEmployeeId, workspaceRows, nextPermissionRows);

  for (const [permissionCode, state] of Array.from(desiredPermissions.entries())) {
    await setPermissionState(targetEmployeeId, permissionCode, state, employeeId(authContext));
  }
...
export async function applyAccountPreset(
  ...
  const authContext = await requireAccountManager();
  await loadTargetEmployee(targetEmployeeId);
  const presetCode = typeof input.presetCode === 'string' ? input.presetCode : '';
  ...
  const presetPermissions = new Set(preset.permissions);
  ALL_PERMISSION_CODES.forEach((permissionCode) => {
    desiredPermissionMap.set(permissionCode, presetPermissions.has(permissionCode) ? 'ALLOW' : 'NONE');
  });

  const { workspaceRows, permissionRows } = await loadAccountRows();
  const nextWorkspaceRows = makeNextWorkspaceRows(workspaceRows, targetEmployeeId, preset.workspaces);
  const nextPermissionRows = makeNextPermissionRows(permissionRows, targetEmployeeId, desiredPermissionMap);
  await assertNotRemovingLastAdministrator(targetEmployeeId, nextWorkspaceRows, nextPermissionRows);

  for (const workspace of ['STAFF_WORKSPACE', 'ADMIN_WORKSPACE'] as WorkspaceCode[]) {
    if (preset.workspaces.includes(workspace)) {
      await grantWorkspace(targetEmployeeId, workspace, employeeId(authContext));
    } else {
      await revokeWorkspace(targetEmployeeId, workspace);
    }
  }

  for (const [permissionCode, state] of Array.from(desiredPermissionMap.entries())) {
    await setPermissionState(targetEmployeeId, permissionCode, state, employeeId(authContext));
  }
...
export async function revokeAccountAccess(targetEmployeeId: string): Promise<AdminAccountActionResult> {
  const authContext = await requireAccountManager();
  await loadTargetEmployee(targetEmployeeId);
  const { workspaceRows, permissionRows } = await loadAccountRows();
  const nextWorkspaceRows = makeNextWorkspaceRows(workspaceRows, targetEmployeeId, []);
  const desiredPermissionMap = new Map<PermissionCode, PermissionEditorState>();
  ALL_PERMISSION_CODES.forEach((permissionCode) => desiredPermissionMap.set(permissionCode, 'NONE'));
  const nextPermissionRows = makeNextPermissionRows(permissionRows, targetEmployeeId, desiredPermissionMap);
  ...
  for (const permissionCode of ALL_PERMISSION_CODES) {
    await setPermissionState(targetEmployeeId, permissionCode, 'NONE', employeeId(authContext));
  }
