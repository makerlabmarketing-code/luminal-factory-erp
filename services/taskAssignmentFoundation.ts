...
export const TASK_ASSIGNMENT_UPDATE_KEYS = new Set([
  "title",
  "description",
  "phaseId",
  "parentTaskId",
...
function optionalPositiveInteger(
  ...
  if (!Number.isInteger(numericValue) || numericValue <= 0) {
    throw new TaskAssignmentValidationError([
      { field, message: "Giá trị phải là số nguyên dương." },
    ]);
  }
...
function optionalText(
  ...
  if (typeof value !== "string") {
    throw new TaskAssignmentValidationError([
      { field, message: "Giá trị phải là chữ." },
    ]);
  }
  ...
  if (text.length > maxLength) {
    throw new TaskAssignmentValidationError([
      { field, message: `Nội dung tối đa ${maxLength} ký tự.` },
    ]);
  }
...
function requiredTitle(value: unknown): string {
  ...
  if (!title) {
    throw new TaskAssignmentValidationError([
      { field: "title", message: "Vui lòng nhập tên công việc." },
    ]);
  }
...
function optionalIsoDate(value: unknown): string | null | undefined {
  ...
  if (Number.isNaN(timestamp)) {
    throw new TaskAssignmentValidationError([
      { field: "deadline", message: "Hạn hoàn thành không hợp lệ." },
    ]);
  }
...
export function isTaskAssignmentStatus(
  value: unknown,
): value is TaskAssignmentStatus {
...
export function validateTaskAssignmentCreatePayload(
  body: Record<string, unknown>,
): TaskAssignmentCreatePayload {
  ...
  return {
    ...
    parentTaskId:
      optionalPositiveInteger(body.parentTaskId, "parentTaskId") ?? null,
    assigneeEmployeeId:
...
export function validateTaskAssignmentUpdatePayload(
  body: Record<string, unknown>,
): TaskAssignmentUpdatePayload {
  ...
  if (body.description !== undefined)
    payload.description =
      optionalText(body.description, "description", 2000) ?? null;
  if (body.phaseId !== undefined)
    payload.phaseId = optionalPositiveInteger(body.phaseId, "phaseId") ?? null;
...
export function validateTaskAssignmentAssignPayload(
  body: Record<string, unknown>,
): TaskAssignmentAssignPayload {
  assertKnownFields(body, TASK_ASSIGNMENT_ASSIGN_KEYS);

  if (!Object.prototype.hasOwnProperty.call(body, "assigneeEmployeeId")) {
    throw new TaskAssignmentValidationError([
      {
        field: "assigneeEmployeeId",
        message: "Vui lòng chọn người được giao hoặc chọn bỏ giao việc.",
      },
    ]);
  }
...
export function validateTaskAssignmentStatusPayload(
  body: Record<string, unknown>,
): TaskAssignmentStatusPayload {
  ...
  if (!isTaskAssignmentStatus(body.status)) {
    throw new TaskAssignmentValidationError([
      { field: "status", message: "Trạng thái công việc không hợp lệ." },
    ]);
  }
