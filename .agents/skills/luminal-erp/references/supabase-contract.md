...
After `DATABASE_TCP_UNAVAILABLE` is confirmed, stop retrying `psql`, `supabase db push`, `supabase db query`, and pooler probes from Codex Cloud. Prefer Supabase GitHub Integration delivery, Management API checks, CLI metadata, and read-only validation when available. Continue application-only work from reviewed repository artifacts when possible, and record the limitation in the roadmap, handoff, and remediation ledger. Do not weaken schema review, RLS review, rollback, validation, compatibility/backfill, or live-approval requirements.

For reviewed production migrations, use the configured Supabase GitHub Integration path as canonical delivery: prepare forward SQL, rollback SQL, validation SQL, compatibility/backfill artifacts, documentation, and tests; commit only approved forward SQL under `supabase/migrations/`; keep rollback and validation SQL outside the migrations directory; create or update the pull request; then stop at the production approval boundary. Production execution occurs only after the approved PR merges into protected `main` and the Supabase GitHub Integration applies the migration. Do not require a separate "Apply migration" task for a complete reviewed package. Never place draft or unapproved SQL in `supabase/migrations/`, and never expose `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD`, connection strings, database passwords, or `.pgpass` contents. Keep Supabase access token and database password values as setup-only secrets rather than ordinary environment variables.
