...
describe('system metadata dropdown fallbacks', () => {
  it('keeps financial ledger dropdowns usable when system_metadata rows are empty or missing', () => {
    expect(normalizeSystemMetadataOptions(null, DEFAULT_FINANCIAL_TRANSACTION_TYPES)).toEqual(
      DEFAULT_FINANCIAL_TRANSACTION_TYPES
    );
    expect(normalizeSystemMetadataOptions([], DEFAULT_CAPITAL_CONTRIBUTION_TYPES)).toEqual(
      DEFAULT_CAPITAL_CONTRIBUTION_TYPES
    );
    expect(normalizeSystemMetadataOptions([{ code: '', label: 'Blank' }], DEFAULT_FINANCIAL_TRANSACTION_TYPES)).toEqual(
      DEFAULT_FINANCIAL_TRANSACTION_TYPES
    );
  });
  ...
  it('exposes default categories for the metadata management screen', () => {
    expect(DEFAULT_SYSTEM_METADATA_CATEGORIES.map((category) => category.name)).toContain(
      FINANCIAL_TRANSACTION_TYPE_METADATA_NAME
    );
    expect(DEFAULT_SYSTEM_METADATA_CATEGORIES.every((category) => category.isFallback)).toBe(true);
  });
  ...
  it('uses fallback metadata in the capital and metadata pages without writing seed data', () => {
    ...
    expect(capitalPage).toMatch(/normalizeSystemMetadataOptions\(meta\?\.data, DEFAULT_FINANCIAL_TRANSACTION_TYPES\)/);
    expect(capitalPage).toMatch(/normalizeSystemMetadataOptions\(contribMeta\?\.data, DEFAULT_CAPITAL_CONTRIBUTION_TYPES\)/);
    expect(metadataPage).toMatch(/DEFAULT_SYSTEM_METADATA_CATEGORIES/);
    expect(metadataPage).toMatch(/Danh mục mặc định chỉ dùng để chống trống dropdown/);
    expect(metadataPage).not.toMatch(/upsert\(|insert\(DEFAULT_SYSTEM_METADATA_CATEGORIES/);
  });
});
