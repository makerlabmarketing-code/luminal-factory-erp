...
describe('Corrective Slice 1 project and workspace authorization contracts', () => {
  it('allows System Owner to use protected global project and phase access without self-revoking owner semantics', () => {
    ...
    const projectAuth = source('services/server/projectMembershipAuthorization.ts');
    const accountManagement = source('services/server/adminAccountManagement.ts');
  ...
  it('keeps Project Manager access membership-scoped when no global admin project permission exists', () => {
    expect(canProjectRolePerformPhaseAction('PROJECT_MANAGER', 'PHASE_VIEW')).toBe(true);
    expect(canProjectRolePerformPhaseAction('PROJECT_MANAGER', 'PHASE_EDIT')).toBe(true);
    expect(canProjectMembershipPerformAction('PROJECT_MANAGER', 'PROJECT_EDIT')).toBe(true);
  ...
  it('keeps Creative Lead and Staff-style contributors view-only at the project membership boundary', () => {
    ...
    expect(canProjectRolePerformPhaseAction('CREATIVE_LEAD', 'PHASE_EDIT')).toBe(false);
    expect(canProjectRolePerformPhaseAction('CONTRIBUTOR', 'PHASE_VIEW')).toBe(true);
    expect(canProjectRolePerformPhaseAction('CONTRIBUTOR', 'PHASE_ASSIGN')).toBe(false);
  ...
  it('does not expose raw database errors from phase loading route responses', () => {
    ...
    expect(phaseRoute).not.toMatch(/supabase_error_message|supabase_error_hint|supabase_error_details/);
  });
  ...
  it('keeps Staff Workspace and Admin Workspace independent without deleting Auth accounts', () => {
    ...
    expect(accountManagement).toMatch(/if \(staffWorkspace\) await grantWorkspace\(targetEmployeeId, 'STAFF_WORKSPACE'/);
    expect(accountManagement).toMatch(/else await revokeWorkspace\(targetEmployeeId, 'STAFF_WORKSPACE'\)/);
    expect(accountManagement).toMatch(/if \(adminWorkspace\) await grantWorkspace\(targetEmployeeId, 'ADMIN_WORKSPACE'/);
    expect(accountManagement).toMatch(/else await revokeWorkspace\(targetEmployeeId, 'ADMIN_WORKSPACE'\)/);
    expect(accountManagement).not.toMatch(/deleteUser|admin\.delete/);
    expect(accountClient).toMatch(/Cấp Staff Workspace|Thu hồi Staff Workspace/);
    expect(accountClient).toMatch(/Cấp Admin Workspace|Thu hồi Admin Workspace/);
