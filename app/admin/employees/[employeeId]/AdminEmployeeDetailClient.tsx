...
type Draft = { fullName: string; email: string; phone: string; title: string; department: string; employmentStatus: string; bankName: string; bankAccountNumber: string };
...
const tabFields: Record<EditableTab, Array<keyof Draft>> = {
  overview: ['fullName', 'email', 'phone'], job: ['title', 'department', 'employmentStatus'], finance: ['bankName', 'bankAccountNumber'],
};
const apiNames: Record<keyof Draft, string> = {
  fullName: 'fullName', email: 'email', phone: 'phone', title: 'title', department: 'department', employmentStatus: 'employmentStatus', bankName: 'bankName', bankAccountNumber: 'bankAccountNumber',
};

function draftFrom(data: EmployeeDetailDto): Draft {
  return { fullName: data.fullName, email: data.email || '', phone: data.phone || '', title: data.title || '', department: data.facilityCode || '', employmentStatus: data.employmentStatus || 'ACTIVE', bankName: data.bankName || '', bankAccountNumber: data.bankAccountNumber || '' };
}
function Field({ label, value }: { label: string; value: string | number | null }) {
  return <div className="rounded-lg border border-slate-800 bg-slate-950/40 p-4"><p className="text-[10px] font-bold uppercase text-slate-500">{label}</p><p className="mt-2 text-sm font-semibold text-slate-100">{value || 'Chưa cập nhật'}</p></div>;
}
...
export default function AdminEmployeeDetailClient({ initialData }: { initialData: EmployeeDetailDto }) {
  ...
  const runAccountAction = async () => {
    if (!accountAction || accountActionSaving) return;
    setAccountActionSaving(true);
    try {
      const response = await fetch(`/api/admin/employees/${data.employeeId}/${accountAction.path}`, { method: 'POST', credentials: 'include', cache: 'no-store' });
      const result = await response.json().catch(() => ({})) as ApiResponse;
    ...
    } finally { setAccountActionSaving(false); }
  };
