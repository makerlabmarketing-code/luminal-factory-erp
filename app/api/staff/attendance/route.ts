    ...
    if (action === 'check_out') {
      if (isAttendanceMultiCheckEnabled() && openRecord) {
        const data = await runMultiCheckMutation({
          action,
          workDate: openRecord.work_date,
          shiftName: openRecord.shift_name,
          checkTime: normalizeTimeValue(timeStr) || timeStr,
        });
        ...
        return NextResponse.json({
          ...
          message: `ÄÃ£ tan ca [${openRecord.shift_name}] thÃ nh cÃ´ng.`,
          record: data,
    ...
    if (isAttendanceMultiCheckEnabled()) {
      ...
      return NextResponse.json({
        ...
        message: existingShift
          ? `ÄÃ£ tiáº¿p tá»¥c ca [${currentShift}] lÃºc ${timeStr}.`
          : `ÄÃ£ ghi nháº­n [${currentShift}] lÃºc ${timeStr}.`,
        record: data,
        capabilities: { multiCheckEnabled: true },
      });
    }
