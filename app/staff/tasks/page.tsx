...
export default function StaffTasksPage() {
  ...
  const loadTasksData = async () => {
    ...
    try {
      // 1. Định danh đúng thợ vận hành qua Token URL
      ...
      // 2. Tải chính xác dữ liệu từ bảng system_settings theo đúng cấu trúc xưởng JSON
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-4 text-slate-100 bg-slate-950 min-h-screen pb-12 font-sans select-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-[65vh] items-start pt-2">
        ...
        {/* 🧩 BẢNG PHẢI: PIPELINE GIAI ĐOẠN CHI TIẾT */}
                      ...
                      {myTasksInPhase.map((t: any) => {
                        ...
                        return (
                          <div key={t.__globalIdx} className="bg-slate-900 border border-slate-850 p-3 rounded-xl flex flex-col gap-2.5">
                            ...
                            <button type="button" onClick={() => handleSaveSpecificTask(phase, t.__globalIdx)} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider flex justify-center items-center gap-1 shadow transition cursor-pointer"><Save size={12}/> Lưu Cập Nhật Ca Việc</button>
                          </div>
                        );
                      })}
                    </div>
