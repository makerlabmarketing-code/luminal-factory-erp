...
export default function StaffExpensesPage() {
  ...
  const handleSubmitExpense = async () => {
    ...
    if (!expCategory.trim() || !expAmount) return showToast('Thiếu thông tin', 'Vui lòng điền đủ tên vật tư và số tiền chi!', 'error');
  ...
  if (loading) return <div className="text-center p-12 text-xs text-slate-500 font-mono bg-slate-950 min-h-screen flex flex-col items-center justify-center gap-2"><RefreshCcw className="w-4 h-4 animate-spin text-emerald-500 mx-auto mb-2"/> Đang tải dữ liệu đối soát...</div>;
    ...
    <div className="p-4 max-w-7xl mx-auto space-y-4 text-slate-100 bg-slate-950 min-h-screen pb-12 font-sans select-none">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3 text-xs shadow-xl">
          ...
          <div><label className="text-slate-400 font-bold">Đường dẫn link chứng từ hóa đơn Bill:</label><input type="text" placeholder="Dán link ảnh đối soát..." className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 mt-1.5 focus:outline-none text-blue-400 font-mono" value={expBillUrl} onChange={(e) => setExpBillUrl(e.target.value)} /></div>
          <button onClick={handleSubmitExpense} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black p-3.5 rounded-xl flex justify-center gap-1.5 transition text-xs shadow-md uppercase tracking-wider mt-2 cursor-pointer"><Save className="w-4 h-4" /> Nộp phiếu hoàn ứng</button>
        </div>

        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
          <div className="px-4 py-3 border-b border-slate-800 bg-slate-950/40 text-[10px] font-bold uppercase text-slate-400 flex justify-between items-center"><span>Sổ đối soát quỹ chi cá nhân của tôi</span></div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60 font-medium text-[11px]">
                {expenses.length === 0 ? (
                  <tr><td colSpan={3} className="p-6 text-center text-slate-500 font-mono italic">Bạn chưa phát sinh khoản kê khai vật tư nào.</td></tr>
                ) : expenses.map(e => (
                  <tr key={e.id} className="hover:bg-slate-950/10 transition">
                    <td className="p-3">
                      <p className="font-bold text-slate-200">{e.category}</p>
                      {e.bill_url && <a href={e.bill_url} target="_blank" rel="noreferrer" className="text-blue-400 flex items-center gap-0.5 mt-1 text-[10px] hover:underline font-mono"><ImageIcon className="w-3 h-3"/> Mở chứng từ ảnh</a>}
                    </td>
