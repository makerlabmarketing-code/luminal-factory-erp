-- DRAFT ONLY. Requires LIVE_APPROVAL_REQUIRED and operator preflight before execution.
-- Expected business-row mutations: 0. Creates/updates only the private Storage bucket definition.
begin;
...
-- Deliberately add no anon/authenticated storage.objects policies. Upload, signed
-- URL creation, replacement, and cleanup use the authorized server boundary.
commit;
