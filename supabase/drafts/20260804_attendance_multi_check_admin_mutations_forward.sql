...
begin
  ...
  if to_regclass('public.attendance_operation_audit') is not null
     or to_regclass('public.attendance_employee_date_shift_active_idx') is not null
     or to_regprocedure('public.attendance_audit_state(public.attendance)') is not null
     or to_regprocedure('public.staff_attendance_multi_mutation(text)') is not null
     or to_regprocedure('public.admin_attendance_mutation(text,bigint,bigint,date,text,time without time zone,time without time zone,text,uuid)') is not null then
    raise exception 'Attendance multi-check precondition failed: package-owned rollout objects already exist';
  end if;
...
create unique index attendance_employee_date_shift_active_idx
  on public.attendance(employee_id, work_date, shift_name)
  where cancelled_at is null;

create table public.attendance_operation_audit (
...
begin
  ...
  -- Serialize all Staff attendance transitions for this employee and own every timestamp.
...
revoke all on function public.staff_attendance_multi_mutation(text) from public, anon;
grant execute on function public.staff_attendance_multi_mutation(text) to authenticated;
