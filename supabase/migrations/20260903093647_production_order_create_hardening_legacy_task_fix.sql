...
begin
  ...
  for v_stage in select value from jsonb_array_elements(v_expected_stages) loop
    ...
    values (v_project_id, v_stage->>'name', (v_stage->>'sequence')::integer, case when (v_stage->>'sequence')::integer = 1 then 'ACTIVE' else 'LOCKED' end)
  ...
  for v_stage in select value from jsonb_array_elements(v_expected_stages) loop
    ...
    for v_task in select value from jsonb_array_elements(v_stage->'tasks') loop
      insert into public.tasks(
        project_id, phase_id, project_name, assigned_to, current_phase, title, status,
        created_by_employee_id, updated_by_employee_id
      )
      values (
        v_project_id,
        (select phase_id from public.production_stages where id = v_stage_id),
        (select project_name from public.projects where id = v_project_id),
        '',
        'IN_PROG',
        v_task->>'title',
