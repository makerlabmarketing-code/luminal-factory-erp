...
| 6 | Attendance recovery | `READY_FOR_OPERATOR` | Facility verification PASS | `supabase/drafts/20260728_attendance_recovery_pre_run.sql` | `supabase/migrations/20260715073600_attendance_recovery_rls.sql` (tracked; never rerun outside migration history) | `supabase/validation/20260715073600_attendance_recovery_rls_validation.sql` | `supabase/rollbacks/20260715073600_attendance_recovery_rls_rollback.sql` | Attendance recovery RLS boundary; rollout 0 rows | Server route requires Admin Workspace + `ATTENDANCE_MANAGE`; own attendance remains independent of membership | Keep `FACILITY_ACTIVE_STATE_ENABLED=false` and `ATTENDANCE_RECOVERY_ENABLED=false` until their respective PASS evidence is retained | Own check-in/out, current/stale open-shift state, history, facility label, failure/retry, and submission locking remain usable; admin UI stays read-only while disabled; rollback on own-row access regression or authorization bypass |
...
| 9 | Payroll | `BLOCKED_BY_BUSINESS_DECISION` | Attendance/facility verification and settlement revision policy approval | Not yet approved | Not yet approved | Not yet approved | Not yet approved | Proposed monthly settlement/snapshot/audit objects; row change depends on first settlement month | Admin confirmation; employee own snapshot read; immutable settlement or approved revision | Keep payroll settlement capability disabled | Official shift boundaries, duplicate settlement rejection, own salary isolation; no package runs before decision |
