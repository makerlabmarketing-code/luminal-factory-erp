...
| 6 | Attendance recovery | `READY_FOR_OPERATOR` | Facility verification PASS | Migration preflight embedded in tracked package | `supabase/migrations/20260715073600_attendance_recovery_rls.sql` (tracked; never rerun outside migration history) | `supabase/validation/20260715073600_attendance_recovery_rls_validation.sql` | `supabase/rollbacks/20260715073600_attendance_recovery_rls_rollback.sql` | Attendance recovery policy/audit boundary; rollout 0 rows | Capability-gated recovery, reason required, immutable audit; own attendance remains independent of membership | Keep recovery capability disabled until PASS | Own check-in/out and history remain usable; recovery rejects missing reason; rollback on own-row access regression |
