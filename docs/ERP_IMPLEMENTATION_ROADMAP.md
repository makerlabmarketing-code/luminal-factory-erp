...
**Repository baseline:** `0169f55` (`style: unify functional admin list and detail surfaces (#73)`)
...
| 9. Facility directory | `APPLICATION_COMPLETE` | `LIVE_OPERATOR_VERIFICATION_REQUIRED` | Server-owned reviewed/legacy projections, stable-code/name resolution, disabled mutation capability, tests, tracked migration, validation, and rollback artifacts exist. | Production compatibility audit, migration-history/post-run validation, assigned-facility and inactive-exclusion smoke tests remain; `FACILITY_ACTIVE_STATE_ENABLED` stays false. | Operator begins with the Facility card in the [activation matrix](runtime-gate-activation-matrix.md) and [production runbook](production-runtime-gate-operator-runbook.md). |
| 10. Attendance | `APPLICATION_COMPLETE` | `LIVE_OPERATOR_VERIFICATION_REQUIRED` | Normal Staff attendance remains functional; Admin recovery is capability-gated in `app/api/admin/attendance/route.ts`; calculation, portal, and recovery contract tests exist; tracked RLS/validation/rollback artifacts are registered. | Facility validation precedes Attendance recovery; own-row/admin authorization and recovery smoke tests must PASS. `ATTENDANCE_RECOVERY_ENABLED` stays false. | After Facility PASS, operator follows Attendance recovery in the [activation matrix](runtime-gate-activation-matrix.md) and [production runbook](production-runtime-gate-operator-runbook.md). |
...
The exact next incomplete roadmap item is **Roadmap item 9, Facility directory — live operator verification for `FACILITY_ACTIVE_STATE_ENABLED` Gate 1**.

The repository package is ready, but the operator must first run the read-only compatibility audit and verify migration history/post-run state. The flag remains false/unset unless all package and smoke prerequisites pass. Attendance recovery is the dependent next gate. The project-workflow gate chain is independent and must not be represented as a Facility dependency.
