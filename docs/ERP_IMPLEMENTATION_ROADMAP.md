...
debt and must not be inherited without explicit authorization checks. No forward
migration execution, permission mutation, seed, production mutation, deployment,
or merge was performed.

The review-only schema foundation is now prepared at
...
executed. Draft PR #177 was opened from commit `9d63603`; this continuation
prepares the fixture update in that same PR. Its first Vercel preview is
successful, and no review thread or submitted review was present at the latest
check. Exact next gate: protected review of the updated
package, followed by one execution on an existing no-cost safe non-production
database. Do not create a paid Supabase branch or a second browser-executable
apply RPC. Production delivery, runtime activation, and merge remain separately
approval-gated.
...
| Phase Templates | `ATOMIC_PACKAGE_READY_FOR_PROTECTED_REVIEW` | Draft PR #177, commit `9d63603`; business contract and Option 1 approved; repository package and live-RPC reconciliation prepared 2026-08-21. | Approved specification/preflight; review-only schema forward, checksum-guarded project-create replacement, lifecycle-management RPC, rollback, post-validation, non-production fixture/runbook, security review, and live reconciliation. Nothing is promoted to migrations. | Keep `PHASE_TEMPLATES_ENABLED=false` or unset. | Protected review, then run the packaged rollback-only matrix once on an existing no-cost safe non-production database; expected production rows affected: 0. | Stop on browser table write, second apply RPC, unchecked privileged function, RLS bypass, unapproved seed, existing-project rewrite, partial clone, production mutation, paid branch creation, or substituting production for fixtures. | Repository/static validation and read-only live checksum/security evidence are retained. Required next evidence is the redacted fixture PASS for authorized/denied actor, stale version, contiguous order, role resolution, deadline overflow, forced failure, and zero partial persistence. Rollback preserves project schedule, cloned rows, provenance, and audit. |
...
| 17. Phase Templates | `SAFE_CLOUD_WORK_AVAILABLE` | Option 1 is approved and the review-only schema, exact atomic apply, lifecycle management, rollback/validation, security/application boundary, and live-RPC reconciliation package is complete. The exact safe action is protected review; database fixtures require non-production infrastructure. |
...
Forward execution, runtime activation, production mutation, and deployment
...
After applying the required skips, no production delivery or fixture execution
is safe until protected review passes and a non-production database exists.
...
| 17. Phase Templates | `SAFE_CLOUD_WORK_AVAILABLE` | `ATOMIC_PACKAGE_READY_FOR_PROTECTED_REVIEW` | Option A, all twelve decisions, and follow-up Option 1 are approved. Schema, exact atomic clone, lifecycle management, rollback/post-validation, non-production fixture/runbook, security/application boundary, and live-RPC reconciliation are prepared outside migrations on draft PR #177. | Protected review and execution of the packaged matrix on an existing no-cost non-production database remain; `PHASE_TEMPLATES_ENABLED` stays false/unset. | Review the bounded package, then stop until a safe non-production database is available; do not create a paid branch. Production delivery and activation require separate approval. |
