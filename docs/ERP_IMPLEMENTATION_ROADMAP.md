...
| **17.5A. Project Mutation Server Boundary** | 🔄 | Đang làm. Mục tiêu: chuyển create/update/archive project qua server API/action, validate DTO, kiểm tra `ADMIN_WORKSPACE` + `PROJECT_MANAGE` hoặc project role phù hợp trước khi mở mutation policy. Không triển khai browser mutation trong Project Read RLS rollout. | `Không mở broad UPDATE cho browser client.` | Duyệt thiết kế/server boundary trước mutation RLS. |
| **17.5B. Phase RLS** | ⏳ | Chờ. Chưa tạo policy cho `public.phases`; chưa thay query phase/task trong lượt Project Read RLS. | `Phase RLS chờ sau Project Mutation Server Boundary hoặc duyệt riêng.` | Thiết kế Phase RLS riêng. |
