...
|---|---|---|---|---|---|---|---|
| Attendance stale-row cancellation | `READY_FOR_LOCAL_OPERATOR` | PR #100, `b8a8bfb`; legacy `NULL`/exact-zero guard correction is on `main`. Earlier forward attempts rolled back safely; **no successful cancellation mutation occurred**. | Runbook: `docs/attendance-stale-row-cancellation-operator-runbook.md`; migration `supabase/migrations/20260730024246_attendance_cancellation_audit.sql`; pre-run/forward `supabase/drafts/20260730_attendance_stale_cancellation_{pre_run,forward}.sql`; post-run/package validation `supabase/validation/20260730_attendance_stale_cancellation_{post_run,validation}.sql`. Confirm migration history first. | Keep `ATTENDANCE_RECOVERY_ENABLED=false` or unset. | Updated read-only pre-run (0 rows mutated) → explicit approval → guarded forward exactly once (exactly 1 target row) → post-run → validation → Staff/Admin smoke. | Stop unless target count and employee open-row count are both 1, each provisional total is `NULL` or exactly zero, Payroll references are 0, actor is ACTIVE, `ADMIN_WORKSPACE` and `ATTENDANCE_MANAGE` pass, active deny count is 0, and grants match; stop on any count/smoke failure. | Retain migration history, pre-run, approval, forward count/audit, post-run, package validation, authorization/RLS and Staff/Admin smoke. Rollback: `supabase/rollbacks/20260730_attendance_stale_cancellation_rollback.sql`, only with retained audit ID and approved reason. |
...
This repair does not change task assignment, transition, persistence, permission, or workflow business rules. It does not reopen any operator package or decision gate. No SQL, production-row inspection, runtime flag change, live email, deployment, or merge occurred. Attendance stale-row cancellation and Finance atomic RPC remain `READY_FOR_LOCAL_OPERATOR`; Employee Profile schema extension, Phase Templates, and email history schema/RLS/archive/retry remain `BLOCKED_BY_BUSINESS_DECISION`. After this newly evidenced defect was resolved, no further item is classified `SAFE_CLOUD_WORK_AVAILABLE`; the exact next safe action is to process new repository or review evidence if it becomes accessible, otherwise stop at the existing operator/business-decision boundaries.
