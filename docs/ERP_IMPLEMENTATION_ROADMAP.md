...
| **17.5E. Task Assignment Employee ID Foundation** | ✅ | Hoàn thành ngày 2026-07-20. Đã áp dụng migration Task Assignment Foundation qua Supabase Management API, backfill deterministic fields, xóa 2 legacy trial task rows đã được duyệt destructive repair, validation schema/RLS/data-integrity PASS, và wire server repository thật sau feature gate `TASK_ASSIGNMENT_FOUNDATION_ENABLED`. | `Task Assignment Foundation hoàn thành. Real persistence đã wire sau migration gate; chưa deploy.` | Project workflow template foundation. |
