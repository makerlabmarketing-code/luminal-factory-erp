...
- **Current phase:** Phase 2 - Task Assignment Foundation / Project Detail task workflow
- **Current status:** ✅ Application wiring PASS validation; chờ PR review/deploy
- **Current blocker:** Không có blocker trong codebase; không chạy migration/backfill/RLS trong slice này.
- **Next safe action:** Review PR/deploy với `TASK_ASSIGNMENT_FOUNDATION_ENABLED=true`, live QA task assignment trên Project Detail, sau đó chuyển sang Phase 3 project workflow state machine.
...
- Next safe slice: Phase status/dependency mutation design, migration/rollback/validation SQL, audit-history plan and RLS impact. Stop at LIVE_APPROVAL_REQUIRED before any schema or data mutation.
