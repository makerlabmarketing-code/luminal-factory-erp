...
executed. Because no no-cost ERP non-production database exists, the business
owner explicitly approved direct production delivery on 2026-08-21. The exact
reviewed package was promoted as one atomic migration at
`supabase/migrations/20260821065313_phase_template_release_one.sql`, with no
seed or legacy backfill. PR #177 merged through protected `main` as `0f3fe87`;
the Supabase GitHub Integration applied migration
`20260821065313_phase_template_release_one` exactly once, and the exact merge
commit deployed to Vercel production with status `READY`.

Read-only post-validation passed all retained schema, source-column,
permission, RLS, browser-write denial, anon denial, index, trigger, RPC grant,
atomic-contract, lifecycle-integrity, and empty-catalog checks. Source counts
remained 12 projects, 8 phases, and 0 tasks; the 30-minute Vercel runtime-error
scan was clean. The retained result is
[phase-template-production-delivery-result.md](phase-template-production-delivery-result.md).
Keep `PHASE_TEMPLATES_ENABLED=false` or unset. Runtime activation and any live
template/project smoke remain separately approval-gated.
...
| Phase Templates | `PRODUCTION_MIGRATION_PASS / RUNTIME_FLAG_DISABLED` | PR #177 merged as `0f3fe87`; migration `20260821065313_phase_template_release_one`, read-only post-validation, Vercel production deployment, and runtime-error scan passed. | [Production delivery result](phase-template-production-delivery-result.md); rollback, validation, fixture/runbook, security review, and live reconciliation remain retained. | Keep `PHASE_TEMPLATES_ENABLED=false` or unset. | Stop until separate runtime-activation and bounded production-smoke approval; no template/project fixture is authorized. | Stop on flag drift, browser table write, second apply RPC, authorization bypass, unexpected catalog row, source-count drift, or production error. | Migration history contains one row; six relations/RLS/grants/functions/lifecycle checks pass; catalog is empty; source counts remain 12/8/0. Non-production fixture remains NOT_EXECUTED. |
...
The 2026-08-21 production-direct exception completed only the Phase Template
migration delivery. The non-production fixture remains unexecuted and every
other operator/fixture boundary remains unchanged.
