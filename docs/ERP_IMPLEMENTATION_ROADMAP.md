...
| Item | Current status | Owning delivery | Package files | Runtime flag | Prerequisite and exact next operator gate | Stop conditions | Rollback |
|---|---|---|---|---|---|---|---|
| Attendance stale-row cancellation | `READY_FOR_LOCAL_OPERATOR` | PR #100, `b8a8bfb`; legacy `NULL`/exact-zero guard correction is on `main`. Earlier forward attempts rolled back safely; **no successful cancellation mutation occurred**. | Runbook: `docs/attendance-stale-row-cancellation-operator-runbook.md`; migration `supabase/migrations/20260730024246_attendance_cancellation_audit.sql`; pre-run/forward `supabase/drafts/20260730_attendance_stale_cancellation_{pre_run,forward}.sql`; post-run/package validation `supabase/validation/20260730_attendance_stale_cancellation_{post_run,validation}.sql`. | Keep `ATTENDANCE_RECOVERY_ENABLED=false` or unset. | Confirm migration history, then updated pre-run → explicit mutation approval → guarded forward exactly once → post-run → package validation → Staff/Admin smoke. | Stop unless target count and employee open-row count are both 1, each provisional total is `NULL` or exactly zero, Payroll references are 0, actor authorization passes, and grants are as documented; stop on any affected-row mismatch or smoke failure. | `supabase/rollbacks/20260730_attendance_stale_cancellation_rollback.sql`; use only with the retained cancellation audit ID and approved rollback reason. |
| Finance linked-ledger atomic edit | `READY_FOR_LOCAL_OPERATOR` after finance PR merge (now on `main`) | PR #103, `e82b873`; compensation-safe application fix committed. Browser compensation is failure-safe but **not true atomicity**. | `docs/finance-linked-ledger-atomic-edit-operator-package.md`; pre-run/forward/rollback `supabase/drafts/20260731_finance_linked_ledger_edit_{pre_run,forward,rollback}.sql`; validation `supabase/validation/20260731_finance_linked_ledger_edit_validation.sql`. | No new runtime flag; do not wire/use the RPC until validation passes. | Pre-run → explicit mutation approval → apply approved `SECURITY INVOKER` RPC → validation/grants/RLS → wire/use RPC → CREATE/UPDATE/CANCEL/NONE smoke → forced dependent failure with zero partial persistence. | Stop before forward on any table, column, RLS, policy, ownership, grant, or function invariant failure; stop before wiring on validation failure; stop and roll back on partial persistence or authorization broadening. | `supabase/drafts/20260731_finance_linked_ledger_edit_rollback.sql`. RPC package is prepared and **not executed**. |
...
| Ledger/Reimbursement | `READY_FOR_OPERATOR` / `LIVE_APPROVAL_REQUIRED` | PR #81, `362cc68d`; package `20260728153000`. | Pre-run `supabase/drafts/20260728153000_ledger_reimbursement_pre_run.sql`; forward `supabase/migrations/20260728153000_ledger_reimbursement_workflow.sql`; validation/rollback/smoke/storage files with the same identifier. | Keep `FINANCE_REIMBURSEMENT_ENABLED=false` or unset. | After Attendance/finance gates as scheduled: pre-run, stop for mutation approval, protected delivery, private Storage/RLS review, validation, authorization and smoke. | Stop on count drift, public storage, RLS/grant failure, legacy salary mutation, hard delete, idempotency failure, or payroll-source regression. | `supabase/rollbacks/20260728153000_ledger_reimbursement_workflow_rollback.sql`; export/approval required because rollback drops new workflow data. |
| Payroll | `READY_FOR_OPERATOR` | PR #80, `6090ace`; package `20260728100414`. | Pre-run/smoke under `supabase/drafts/payroll/`; tracked forward and validation/rollback files named `20260728100414_*`. | Keep `PAYROLL_SETTLEMENT_ENABLED=false` or unset. | Attendance/Facility PASS → pre-run → mutation approval/protected delivery → validation/RLS/authorization → explicit first official month → smoke. | Stop on legacy-row drift, own-row isolation failure, unauthorized execution, duplicates, mutable originals, missing audit provenance, or unspecified first month. | `supabase/rollbacks/20260728100414_immutable_monthly_payroll_settlement_rollback.sql`; export and destructive-rollback approval required. |
| ERP transactional email | `READY_FOR_OPERATOR` for one-recipient configuration/smoke; history governance remains `BLOCKED_BY_BUSINESS_DECISION` | PR #95 and PR #104, `8787e21`. | `docs/email-setup.md`; `docs/email-history-business-decision-package.md`. | Keep `EMAIL_DELIVERY_ENABLED=false` or unset until the documented one-recipient smoke passes. | Configure server-only values, stop before the first live send, then execute only the approved one-recipient checklist. | Stop on missing/incorrect server configuration, recipient uncertainty, provider failure, PII/secrets in evidence, or unresolved history-governance work being treated as approved. | Disable `EMAIL_DELIVERY_ENABLED`; follow provider/runbook reversal and do not delete history as rollback. |
| Facility evidence | `LIVE_OPERATOR_VERIFICATION_REQUIRED` | PR #83 plus tracked migration `20260723120000`. | `docs/facility-employee-production-compatibility.md`; audit/validation/rollback files `20260727_facility_employee_compatibility_audit.sql` and `20260723120000_facility_status_code_*`. | Keep `FACILITY_ACTIVE_STATE_ENABLED=false` or unset. | Verify migration history and run read-only compatibility/fixtures before any conditional approved mutation. | Stop on missing/duplicate codes, active-state mismatch, authorization/RLS failure, or Attendance regression. | `supabase/rollbacks/20260723120000_facility_status_code_rollback.sql`. |
| Dashboard fixtures | `READY_FOR_OPERATOR` (read-only fixtures only) | Dashboard application status preserved by the reconciled roadmap; no activation PR is claimed. | `docs/core-erp-functional-stabilization-report.md` and production runbook fixture checklist. | No Dashboard-specific runtime flag. Dependent finance/payroll/attendance flags remain disabled. | After source packages pass, retain empty, populated, denied, and error/retry fixture evidence. | Stop if a fixture requires inventing or mutating production data, exposes raw errors, or disagrees with its authoritative source. | No data rollback: do not create production fixtures; revert only application/runtime changes through their owning packages. |
