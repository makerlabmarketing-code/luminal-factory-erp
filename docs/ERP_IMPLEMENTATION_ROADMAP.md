...
| **20. Batch 3E1: Own-row RLS**                                    |         🔄 | Đã chuẩn bị draft forward/rollback/validation/compatibility/security package cho Staff own-profile `employees` SELECT qua `auth.uid()` → `employees.auth_user_id`, đồng thời giữ RLS trên `attendance` và `attendance_logs`. Chưa chạy SQL, chưa promote migration, chưa backfill.                                                                                                                                                                                                                                                                                                                                                                      | `Review draft own-row RLS package. Chỉ promote qua Supabase GitHub Integration sau duyệt riêng.`                                                                     | Duyệt và rollout own-row RLS slice; sau validation mới tiếp tục payroll/finance authorization.                           |
...
Status: `LIVE_APPROVAL_REQUIRED` after safe artifact preparation. Continued the next approved security roadmap item, Batch 3E1, after Batch 3D2 reached its live approval gate and no connected newly actionable Code Review findings were available.

Completed safe work:

- Prepared draft-only forward SQL to add Staff own-profile SELECT access on `public.employees` through `employees.auth_user_id = auth.uid()` plus active `STAFF_WORKSPACE` access.
- Defensively kept RLS enabled on `public.employees`, `public.attendance`, and `public.attendance_logs` while preserving existing admin employee-view and attendance recovery policies.
- Prepared rollback SQL that removes only the new employees own-profile policy and does not disable RLS or mutate data.
- Prepared read-only validation SQL for employee/attendance RLS, employee own-profile policy shape, absence of broad authenticated employee SELECT policy, and continued attendance own-row policies.
- Prepared compatibility/backfill and security notes confirming no automated backfill, no payroll/finance authorization change, no attendance calculation/source-of-truth change, and no Auth/permission/employee/attendance data mutation.
- Added static regression coverage proving the package remains draft-only and includes forward, rollback, validation, compatibility/backfill, and security evidence.
...
## 2026-07-27 UI/UX polish handoff

Status: ✅ handoff prepared; broad redesign remains outside the functional roadmap run.

- Added `docs/ui-ux-polish-handoff.md` as the bounded review queue for layout, typography, modal sizing, Project Detail, Employee Detail, mobile Attendance, responsive admin pages, loading, dark mode, toasts, button hierarchy, and navigation.
- Recorded the screenshot matrix and review metadata required before approving perceptible UI changes, without using production mutation to manufacture states.
- Functional behavior, business rules, persistence, permissions, and live capability rollout remain authoritative and must not be changed by polish work without a separately proven defect.

## 2026-07-27 GitHub review remediation handoff

Status: ✅ handoff prepared; review inspection and remediation intentionally deferred.

- Added `docs/github-review-remediation-handoff.md` without reading GitHub PR comments, Codex findings, Copilot comments, CodeQL suggestions, or historical remediation notes during this roadmap run.
- The handoff defines the later classification, smallest-safe-fix, regression-test, validation, existing-PR update, and re-check loop.
- No unseen finding is represented as resolved or non-actionable. Current production capability and approval gates must remain explicit throughout the later stabilization pass.

## 2026-07-27 Project creation and detail loading repair

Status: ✅ application repair complete; no production SQL or deployment performed.

- Project codes are now generated by the server as `LF-ddMMyy-NN` using the Luminal business timezone. Creation reads the existing codes for that date, inserts behind the existing unique project-code constraint, and retries only a unique collision so concurrent requests cannot persist the same code.
- Removed editable project-code and colorway-code fields from the create flow. Required fields now expose Vietnamese inline errors, error borders, `aria-invalid`, and first-invalid-field focus instead of silently stopping submission.
- The normalized create response returns the confirmed `projectId` and generated `projectCode`. Successful creation inserts a local project placeholder and no longer triggers a post-create project-list waterfall or navigates from a stale identifier.
