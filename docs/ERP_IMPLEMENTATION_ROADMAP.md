...
| **17.1. Batch 3C4B: RLS Admin Data Access Slice 1** | 🔄 | Đang làm pre-run review. Scope chỉ gồm helper `is_app_admin()`, SELECT policy cho `public.financial_ledger`, rollback SQL, validation SQL và security tests. Chưa chạy SQL. | `Review migration/rollback/validation SQL. Chỉ chạy sau duyệt lần hai. Không mở anon, không policy ALL, không sửa dữ liệu.` | Chờ duyệt pre-run, sau đó rollout SQL riêng và live validation. |
...
- Batch 3C4B RLS slice 1: Đang làm pre-run review. Đã tạo draft migration helper `public.is_app_admin()`, SELECT policy cho `public.financial_ledger`, rollback SQL và validation/security test SQL. Chưa chạy SQL, chưa sửa live schema/RLS/data.
