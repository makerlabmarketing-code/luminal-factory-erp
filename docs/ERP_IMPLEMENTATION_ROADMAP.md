...
creation selector. Full repository validation passed 98 test files / 755 tests,
...
Exact next gate: protected review, followed by the authorization/atomicity matrix
on a safe non-production database.
Do not create a second browser-executable apply RPC. Production delivery,
runtime activation, push/deploy, and merge remain separately approval-gated.
...
| Phase Templates | `ATOMIC_PACKAGE_READY_FOR_PROTECTED_REVIEW` | Business contract and Option 1 approved; repository package and live-RPC reconciliation prepared 2026-08-21; implementation PR not created. | Approved specification/preflight; review-only schema forward, checksum-guarded project-create replacement, lifecycle-management RPC, rollback, validation, security review, and live reconciliation. Nothing is promoted to migrations. | Keep `PHASE_TEMPLATES_ENABLED=false` or unset. | Protected review, then apply only to a safe non-production database for the complete fixture matrix; expected production rows affected: 0. | Stop on browser table write, second apply RPC, unchecked privileged function, RLS bypass, unapproved seed, existing-project rewrite, partial clone, production mutation, or substituting production for fixtures. | Repository/static validation and read-only live checksum/security evidence are retained. Required next evidence: authorized/denied actor, stale version, contiguous order, role resolution, deadline overflow, forced failure, and zero partial persistence. Rollback preserves project schedule, cloned rows, provenance, and audit. |
...
| 17. Phase Templates | `SAFE_CLOUD_WORK_AVAILABLE` | `ATOMIC_PACKAGE_READY_FOR_PROTECTED_REVIEW` | Option A, all twelve decisions, and follow-up Option 1 are approved. Schema, exact atomic clone, lifecycle management, rollback/validation, security/application boundary, and live-RPC reconciliation are prepared outside migrations. | Protected review and the non-production authorization/atomicity matrix remain; `PHASE_TEMPLATES_ENABLED` stays false/unset. | Review the bounded package, then stop until a safe non-production database is available; production delivery and activation require separate approval. |
