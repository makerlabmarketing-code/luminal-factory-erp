...
| 5. Staff Portal and attendance | APPLICATION_COMPLETE / OPERATOR_PRODUCTION_VERIFICATION_REQUIRED | Staff Portal routes render separate attendance/task/expense/profile tabs; normal own-row check-in/check-out is independent from both Project Membership and the recovery flag; Admin attendance stays useful and read-only while recovery is disabled. Repository validation is `PASS`. | Live Gate 2 migration-history, pre-run, post-run, own-row/RLS, and authorized/denied smoke evidence remains; recovery mutation is disabled. | Verify migration history, run the read-only pre-run, retain post-run evidence, complete authenticated production authorization/smoke checks, and keep `ATTENDANCE_RECOVERY_ENABLED=false` until all evidence passes. | No |
...
| Attendance | APPLICATION_COMPLETE / OPERATOR_PRODUCTION_VERIFICATION_REQUIRED | Repository validation is `PASS`; recovery remains false/unset pending retained migration-history, read-only pre-run, post-run, and authenticated production authorization/RLS smoke evidence. |
