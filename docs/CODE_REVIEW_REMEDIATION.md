...
| #12              | `b9ba23d`              | Task creation must be atomic with required side effects                          |       P1 | `services/server/taskAssignmentFoundation.ts`, `createProjectTask`; `supabase/migrations/20260815165046_task_assignment_atomic_create.sql` | ACTIONABLE_FIXED | The application remains fail-closed and invokes one hardened, service-role-only RPC. The reviewed migration performs database-owned actor/project/relationship checks and atomically persists the task plus required side effects. Production migration delivery, fixture verification, and runtime activation remain separately gated. | current branch | Atomic create service mapping; forward, pre/post validation, rollback, roadmap and handoff package. | `tests/task-assignment-foundation.test.ts` — `delivers a hardened service-role-only atomic task-create package`; `tests/runtime-gate-safety-boundaries.test.ts` — browser execute revocation | Focused 12/12 and full 746/746 Vitest, lint, typecheck, build, audit classification, and diff check passed. | current PR | `REVIEW_SOURCE_UNAVAILABLE`; no current GitHub review source or unresolved conversation is exposed in this checkout. | FIXED_AWAITING_PRODUCTION_GATE |
...
| Finding | Classification | Evidence | Remediation / status |
| --- | --- | --- | --- |
| The guarded Attendance stale-row cancellation required retained proof that the approved forward committed once and produced one audited cancellation. | OPERATOR_EVIDENCE_COMPLETE | `C:\Users\tungd\AppData\Local\Temp\attendance-cancellation-forward.txt` records exit code `0` and `COMMIT`; the post-run records exactly one cancelled Attendance row and immutable audit event ID `1`. | Forward execution is complete and must not be replayed. Rollback was not executed and remains separately approval-gated. |
| Post-commit state and package invariants required independent read-only validation. | VERIFIED_PASS | `C:\Users\tungd\AppData\Local\Temp\attendance-cancellation-post-run.txt` is `PASS`; `C:\Users\tungd\AppData\Local\Temp\attendance-cancellation-validation.txt` has exit code `0`, zero invalid cancelled rows, zero cancelled Payroll references, zero forbidden audit mutation grants, and the immutable audit trigger inventory. | Attendance production correction is complete. Employee open-row count and finalized Payroll references are `0`; duplicate state count is `0`; `check_out`, `total_hours`, and `total_salary` remain `NULL`. |
| Attendance must not be marked fully production-complete before authenticated workflow verification. | SMOKE_REQUIRED | The operator approval covered the forward, post-run, and package validation only. | Keep `ATTENDANCE_RECOVERY_ENABLED=false`. The next Attendance gate is production Staff/Admin smoke only. |

No forward retry, rollback, runtime-flag activation, unrelated operator package, application-code change, schema change, RLS/grant mutation, Payroll mutation, deployment, Finance work, or unrelated production query was performed during package-wide validation and evidence recording.

## 2026-08-01 Admin ledger beneficiary and attachment remediation

No open pull request or unresolved PR conversation was returned by the connected GitHub review source. Current source, the roadmap, the finance handoff, the existing Ledger/Reimbursement package, Supabase Storage guidance, and repository validation supplied the review evidence.

| Finding | Classification | Evidence | Remediation / status |
| --- | --- | --- | --- |
| Admin create/update/payment attempted direct browser writes while `financial_ledger` RLS exposed only approved Admin `SELECT`. | ACTIONABLE / P1_PRODUCTION_BLOCKER | `app/admin/capital/page.tsx` called browser `.insert()`/`.update()`; migration `20260713111027` defines only the Admin select policy. | Added server-owned Admin ledger routes and service authorization for `FINANCE_VIEW`, `FINANCE_CREATE`, and `FINANCE_UPDATE`; client errors remain controlled Vietnamese messages and failed forms stay open. |
| Admin list and QR reused `requested_by` as the beneficiary. | ACTIONABLE | The list displayed `requested_by`; QR matched employees by that display name. | Stable payer/beneficiary IDs map independently when the gated schema is active; legacy payer display remains readable, unknown beneficiary is `Chưa xác định`, and QR uses only `beneficiary_employee_id`. |
| Attachment UI claimed support while runtime upload, signed preview, replacement, removal, and cleanup were absent. | ACTIONABLE / LIVE GATE REMAINS | The existing storage package explicitly said runtime did not upload. | Added fail-closed server plumbing with content/MIME/extension validation, stable content-addressed paths, signed URLs, metadata retention states, upload-first replacement, and partial cleanup reporting. `FINANCE_ATTACHMENT_WRITES_ENABLED` must remain false until the private bucket plus a database-atomic active-count invariant and authenticated concurrency/cleanup smoke are approved. Prepared draft-only bucket SQL; nothing was executed. |
| Admin full update replaced creator/idempotency provenance and linked-row compensation could itself fail. | ACTIONABLE / P1_FIXED_LOCALLY | `mutationColumns` was shared by create/update and the server wrote the primary row before its counter-row. | Create-only provenance is now separated from update columns. Ordinary single-row edits remain available; any existing or requested counter-row edit returns a controlled `503` until the approved atomic RPC is active. |
| Environment flag alone could expose missing extended schema or an unavailable/public evidence bucket. | ACTIONABLE / P1_FIXED_LOCALLY | Optional queries trusted only `FINANCE_REIMBURSEMENT_ENABLED`. | Server readiness probes now verify required columns/table and private bucket limits; UI attachment controls use only server-returned capability state. Missing readiness fails closed with Vietnamese `503`. |
| Legacy `bill_url` cannot safely support audited replace/remove semantics. | NOT_APPLICABLE_WITH_CURRENT_SCHEMA | It has no protected metadata identity or storage ownership record. | Preserved render/open compatibility only; no live URL was deleted or rewritten. |
